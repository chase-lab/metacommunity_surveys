...
ddata <- ddata[unique(ddata[, .(local, year, month)])[, .SD[1L], by = .(local, year)], on = .(local, year, month)][, order_month := NULL]

## When a site is sampled twice a month, selecting the first visit ----
ddata <- ddata[unique(ddata[, .(local, year, month, day)])[, .SD[1L], by = .(local, year, month)], on = .(local, year, month, day)][, month := NULL][, day := NULL]
...
ddata[, ':='(
   ...
   scientificName = iconv(scientificName, from = 'UTF-8', to = 'ASCII//translit'),
...
# Metadata ----
meta <- unique(ddata[, .(dataset_id, regional, local, year, latitude = decimalLatitude, longitude = decimalLongitude)])
meta[, ":="(
   taxon = "Fish",
   realm = "Marine",
