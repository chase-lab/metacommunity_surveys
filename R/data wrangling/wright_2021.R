...
ddata <- data.table::setnames(ddata,
                              c("ecosystem", "transect"),
                              c("regional", "local")
)
...
data.table::fwrite(
  ddata[,!c("complete_sampling_month", "complete_sampling_year")],
...
ddata <- ddata[complete_sampling_year == 1L & complete_sampling_month == 1L, .(value = sum(value), regional = unique(regional)), by = .(year, local = local, species)]

## metadata ----
meta <- meta[unique(ddata[,.(dataset_id, regional, local, year)]), on = .(dataset_id, regional, local, year)]
