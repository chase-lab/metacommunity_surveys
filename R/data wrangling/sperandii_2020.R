...
ddata[, local := sub("rev", "", local)]
ddata <- ddata[
   ddata[, data.table::uniqueN(timepoints), by = local][V1 == 2L][, local],
   on = .(local)][, timepoints := NULL]

##meta data ----
meta[, local := sub("rev", "", local)]
meta <- unique(meta[
...
meta[, ":="(
   ...
   gamma_sum_grains = 4L * data.table::uniqueN(local),
