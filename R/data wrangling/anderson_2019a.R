...
ddata[, ":="(
  ...
  year = data.table::fifelse(substr(date, 8, 9) == "97", 1997L, as.integer(paste0("20", substr(date, 8, 9))))
)]
...
)][, gamma_sum_grains := sum(alpha_grain) / 10000L, by = .(regional, year)][local == "Wizard", gamma_bounding_box := 1.73]
