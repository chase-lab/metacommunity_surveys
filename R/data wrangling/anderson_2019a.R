# anderson_2019a - invertebrate density
dataset_id <- "anderson_2019a"
...
ddata[, `Juvenile limpet (<5mm)` := NULL]
...
ddata[, ":="(
   ...
   local = paste(local, exposure, `Tidal Height`, sep = "_"),
...
data.table::fwrite(
   x = meta,
   file = paste0("data/wrangled data/", dataset_id, "/", dataset_id, "_raw_metadata.csv"), row.names = FALSE
)
...
ddata <- unique(ddata[effort == 5L][, .(value = sum(value)), by = .(regional, local, year, species)]) # pooling dates together
