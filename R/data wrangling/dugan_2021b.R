...
data.table::fwrite(ddata[,!c("TAXON_FAMILY", "DATE")], paste0("data/wrangled data/", dataset_id, "/", dataset_id, "_raw.csv"),
                   row.names = FALSE
)
...
ddata <- ddata[TAXON_FAMILY %in% c("Otariidae", "Phocidae")][ddata[, .(nmonth = length(unique(month)), nweek = length(unique(DATE))), by = .(local, year)][nmonth == 12L & nweek == 12L], on = .(local, year)]
