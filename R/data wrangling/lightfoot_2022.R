...
# Standardisation ----
data.table::setnames(ddata, c("zone","spp"),c("regional","species"))
ddata[, local := paste0(site, "_", plot)][, year := as.integer(format(date, "%Y"))][, species := as.character(species)]
...
### computing effort ----
ddata[, effort := length(unique(date)) * length(unique(pit)), by = .(year, regional, local)] # Effort is the minimal number of sampling operations ie the number of pitfall traps * the number of dates per local per year.

### pooling pits and dates ----
ddata <- ddata[, .(value = .N, effort = unique(effort)), by = .(year, regional, local, species)]
