...
data.table::setnames(
   x = ddata,
   old = c("location_type", "site_code", "common_name", "lat", "long", "bird_count"),
   new = c("regional", "local", "species", "latitude", "longitude", "value")
)

ddata[, ":="(
   year = format(ddata$survey_date, "%Y"),
   month = format(ddata$survey_date, "%m"),
   day = format(ddata$survey_date, "%d")
)]

ddata[, ":="(
   latitude := mean(latitude),
   longitude := mean(longitude)), by = .(regional, local)]
...
set.seed(42)
ddata <- ddata[
   ddata[, .(observer = observer[sample(x = seq.len(.N), size = 1L)]),
         by = local],
   on = c("local", "observer")]

# sum bird count over direction, seen, heard, distance ----
ddata[, value := sum(value), by = .(local, year, species)][, observer := NULL]
