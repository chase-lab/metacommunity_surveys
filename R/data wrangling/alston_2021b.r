...
meta[, ":="(
   ...
   gamma_sum_grains_type = "trap",
   gamma_sum_grains_comment = "sum of the trap areas sampled each year per region",
   ...
   gamma_sum_grains = NA_real_ * length(unique(local)),
...
drop_col <-  "date"
data.table::fwrite(ddata[, !..drop_col], paste0("data/wrangled data/", dataset_id, "/", dataset_id, "_raw.csv"),
                   row.names = FALSE
)
...
### changing local to be one level up: wee pool all rebars from a plot together
