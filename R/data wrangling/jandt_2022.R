...
data.table::setnames(
   x = ddata,
   old = c("rs_site", "rs_plot", "surf_area", "taxonname", "cover_perc"),
...
ddata[, local := factor(paste0(local, "!", manipulate, "!"))][
   i  = ddata[, data.table::uniqueN(releve_nr),
              keyby = .(regional, local, year, date)][V1 != 1L],
   on = .(regional, local, year, date),
   j  = local := factor(paste0(local, "_", releve_nr, "_"))]
...
ddata <- ddata[base::grepl(pattern = "!N!", x = local, fixed = TRUE)]
...
data.table::setkey(ddata, dataset_id, regional, local,
                   year, month, day, species)
ddata <- ddata[, .(species = unique(species)), keyby = .(dataset_id, regional,
                                                         local, releve_nr,
                                                         year, month, day)]
...
meta[, ":="(
   ...
   comment_standardisation = factor(
      "The original data provides species cover layer by layer. Code meaning: '0: No layer, 1: Tree layer (uppermost), 2: Tree layer (middle), 3: Tree layer (lowest), 4: Shrub layer (uppermost), 5: Shrub layer (low), 6: Herb layer, 7: Juveniles, 8: Seedling (< 1 year), 9: Moss layer.' In this data set, we created `tree` and `undestorey` layers respectively containing layers 1,2 and 3 and 0, 4, 5, 6, 7 and 8.
