...
## Metadata ----
meta <- unique(ddata[, .(dataset_id, regional, local, longitude, latitude,
                         year, month, day)])
meta[, ':='(
   taxon = "Mammals",
   realm = "Terrestrial",
...
ddata[, local := stringi::stri_extract_first_regex(local, "^[0-9]{1,2}(?=_)")]
...
month_order <- ddata[, data.table::uniqueN(day),
                     keyby = .(dataset_id, month)][,
                                                   j = sum(V1),
                                                   keyby = month][order(-V1)][1L:10L, month]
ddata[, month_order := (1L:10L)[match(month, month_order, nomatch = NULL)]]
...
ddata <- ddata[
   unique(ddata[,
                .(dataset_id, local, year, month)])[, .SD[1L:8L],
...
if (nrow(ddata != 0L)) {
   ...
   ## Metadata ----
   meta[, local := stringi::stri_extract_first_regex(local, "^[0-9]{1,2}(?=_)")]
...
"
      ...
      gamma_sum_grains = sum(alpha_grain)), # should it be multiplied by effort
      keyby = year]
